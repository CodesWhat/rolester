...
- `npm run capture:search-sources` / `npm run capture:board` — bulk-scrape a
  board's listing DOM
- Document and packet PDF export (`npm run export`)
...
```bash
npm run automation -- status                               # current matrix
npm run automation -- consent linkedin --write             # record consent
npm run automation -- enable profile_optimize linkedin --write
npm run automation -- session extension --write            # set provider
```
