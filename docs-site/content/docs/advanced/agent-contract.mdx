...
2. **Verify** with `node src/cli/tracker.mjs --verify`.
3. **Re-render** with `node src/cli/tracker.mjs` so the dashboard hot-reloads.
4. **Log one Activity Pulse event** via `npm run activity -- append`.
